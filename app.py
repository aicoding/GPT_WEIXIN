...
from functions.xlog import test_xlog


openai.api_key = os.environ.get("OPENAI_API_KEY")
# openai.api_base = os.environ.get("OPENAI_API_BASE")

modelId = os.environ.get("OPENAI_MODEL") or "gpt-4"
...
def get_token_count(conversation):
    encoding = tiktoken.encoding_for_model(modelId)
...
async def on_message(user_message: object):
    ...
    while cur_iter < MAX_ITER and not is_stop:
        ...
        try:
            ...
            new_delta = stream_resp.choices[0]['message']
            if is_stop:
                is_stop = True
                cur_iter = MAX_ITER
                break
...
async def start_chat():
    content = '''
    1.You are now a code interpreter in the Jupyter Notebook environment. When you encounter code that needs to be processed, please use Python code to analyze and solve the corresponding problems. You can solve problems step by step and can also use variables generated from previous code at any time.
    2.You can obtain configuration metric and data details of a metric from xlog,
    3.Try to avoid printing terminal data with more than 2000 characters. Before considering using operations like print, make sure to first check if the content to be printed is too large. If it is, please store it in a file for later use by subsequent programs.
    '''
    language = os.environ.get("LANGUAGE") or "chinese"
    cl.user_session.set(
        "message_history",
...
async def on_action(action):
    await cl.Message(content=f"Executed {action.value},需要给个具体时间段进行查询，时间段不宜过大").send()
    # Optionally remove the action button from the chatbot user interface
    # await action.remove()

@cl.on_message
async def run_conversation(user_message: object):
    if '/xlog' == str(user_message):
        await test_xlog()
        return
